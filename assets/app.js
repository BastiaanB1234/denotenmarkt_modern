$(function() {
  ...
  $('#nav').mmenu({
      ...
      searchfield : {
        add: true,
        search: true,
        noResults: '<a href="/search?q=" class="search_link">' + "Uitgebreide zoekopdracht \u0026#8594;" + '</a>'
      },
  ...
  $("#cart_form input[type='number']").change(function() {$("#cart_form").submit(); });
    ...
    $("#checkout, #cart input[type='submit']").on('click', function() {
      if ($(this).parents('form').find('.tos_agree').is(':checked')) {
        $(this).submit();
      } else {
        alert("Je moet met de voorwaarden akkoord gaan om te kunnen bestellen.");
        return false;
      }
    });
    ...
    $cf.on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
              $cf.prev('.message').html("Bedankt voor j aanmelding op onze nieuwsbrief!");
            }
        });
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (data) {
              $notify_form.fadeOut("slow", function(){
                $nf.prev('.message').html("Bedankt! We laten je weten wanneer dit product weer beschikbaar is!");
              });
            }
        });
      } else {
        $nf.prev('.message').html("Geef s.v.p. een geldig email adres");
      }  
      e.preventDefault();
    });
  });  
  ...
  function refreshCart(cart) {
    ...
    if ($cartBtn) {
      ...
      if (cart.item_count == 0) {
        $cart.append('<li class="mm-label empty_cart"><a href="/cart">Je winkelmandje is leeg</a></li>');
        $cartCount.remove();
      } else {
        ...
        cart_items_html += '<li class="mm-label">' +
          ...
          '<li class="mm-subtitle clearfix">';
            ...
            cart_items_html += '<textarea id="note" name="note" rows="2" placeholder="Noot" class="clearfix"></textarea>';
          

          
            cart_items_html += '<p class="tos"><input type="checkbox" class="tos_agree" id="sliding_agree" /><label class="tos_label" for="sliding_agree"> ' +
              "Ik ga akkoord met de voorwaarden vermeld op deze site (onderaan elke pagina)" +
            '</label></p>';
          

          cart_items_html += '<input type="submit" class="action_button right" value="Afrekenen" />' +
            '<a href="/cart" class="action_button edit_cart left">Verander</a>' +
          '</li>';
    ...
    $(".product_form").submit(function(e) {
      ...
      $.ajax({
        ...
        success: function(itemData) {
          $addToCartBtn.find('span').text("Toegevoegd").removeClass('zoomOut').addClass('fadeIn');
        ...
        error: function(XMLHttpRequest) {
          ...
          $addToCartBtn.removeAttr('disabled').removeClass('disabled');
          $addToCartBtn.find('span').text("In het winkelmandje").removeClass('zoomOut').addClass('zoomIn');
        }
      });
                          ...
                          if(data.meta.code === 200 && data.data.length) {
                            ...
                            for(var i = 0; i < data.data.length; i++) {
                              ...
                              if(plugin.settings.list) {
                                var item="<div class=\"one-fifth column\">"+item+"</div>";
                              }
